...
# ╔═╡ 0c35cd94-b120-11ed-39b9-63f98b2d4b66
begin
    using CairoMakie, PlutoUI, MRIReco, MRICoilSensitivities, MRIFiles, QuantitativeMRI, Statistics, NIfTI
end

# ╔═╡ 51725380-d445-47ae-9a54-05d5e3611b58
md"# Introduction

This notebook will reconstruct the TI_1, TI_2, MP2RAGE and T1 maps of this sequence :
https://github.com/aTrotier/SEQ\_BRUKER\_A\_MP2RAGE\_CS\_PUBLIC

It has been tested for :
- v0.52
- Fully sampled acquisition where Ny = ETL (you should see an image on the Bruker scanner)

In order to work, an additionnal files is required

---
## Packages
Multiple packages are required, the package versions are stored into this notebook :
- CairoMakie : for plot
- MRIReco : reconstruction
- MRIFiles : reading Bruker rawdatasets
- QuantitativeMRI : create T1 maps from MP2RAGE acquisition
- NIfTI : output images format"

# ╔═╡ 1e68c7eb-7fc0-4867-bd10-21077e7bf7b6
md"## Parameters to update before reconstruction
You should edit the following cells :
- `fid_path` should point to the bruker folder experiment
- `ni_folder` should point to the folder where the nifti should be stored
- `ni_basename` is the name of the nifti files"

# ╔═╡ a004ad9d-5ab1-47ec-a5b2-d0b6425d1da5
begin
	fid_path = "/Users/aurelien/Documents/GitHub/SEQ_BRUKER_A_MP2RAGE_CS_PUBLIC/DATA/8"
	
	ni_folder="/Users/aurelien/Documents/GitHub/SEQ_BRUKER_A_MP2RAGE_CS_PUBLIC/DATA"
	ni_basename = "ni_"

	sl = 50 # default slice to show
end

# ╔═╡ a7588a2a-f4b5-4e46-8263-f492c7e73af0
"""
RawAcquisitionData_MP2RAGE_CS(b::BrukerFile)

	Specific function that reorder the lines to create a RawAcquisition Object in the right format
"""
...
# ╔═╡ 7fc4c6ec-a1f8-4984-b5bd-edf913bd56d3
md"# Load and show 2dseq 

if the images is reconstructed directly on the scanner (Ny = ETL) we can see the images 

But because it is a magnitude images we can't performed the MP2RAGE combinaison and then compute the T1 map"

# ╔═╡ 78747860-58b4-4fab-993c-1f38f74b3754
begin
	b=BrukerFile(fid_path)
	im_sos = recoData(b,1)
	SpatialResolution = Tuple(parse.(Float32,b["PVM_SpatResol"]))
end;

# ╔═╡ 826d5bb7-d5b8-4c84-8798-d1829105a9cf
md"slider that control the slice"

# ╔═╡ fcaf746f-108b-44b9-a7f8-cdf89f5d2fe9
@bind sl_1 PlutoUI.Slider(1:1:size(im_sos,3), default=ceil(Int,size(im_sos,3)/2))

# ╔═╡ f929c1b6-1c1c-48b3-a607-24e874702285
let
	f = Figure()
	heatmap(f[1,1],im_sos[:,:,sl_1,1])
	heatmap(f[1,2],im_sos[:,:,sl_1,2])
	f
end

# ╔═╡ 282b6e60-78c6-4d49-b16d-2688d285c94e
md"# MP2RAGE reconstruction from FID rawdata

### 1) Perform the reconstruction from FID in order to get complex image"

# ╔═╡ a434e90f-b323-4ce5-84fc-38742b298ec4
begin
	...
	sens = estimateCoilSensitivities(Ireco[:,:,:,2:2,:,1:1], 1.e-2)
	Icomb = sum(conj.(sens).*Ireco,dims=5) ./sum(abs.(sens).^2,dims = 5)
end;

# ╔═╡ 87aa1b59-c9f1-492b-9a6b-04d7e698c03a
heatmap(abs.(Icomb[:,:,sl_1,2,1,1]))

# ╔═╡ f3043546-969d-409f-93b1-4e882764c41a
md"### 2) Average the NR"

# ╔═╡ ef5e16e6-7fcb-4257-ab10-45479fe84b86
Imean = mean(Icomb[:,:,:,:,:,:],dims = 6)[:,:,:,:,1,1];

# ╔═╡ 3a5c1260-b398-4d3a-b3a7-2f028b1295f2
begin
	# apply MP2RAGE
    MP2 = mp2rage_comb(Imean)
...
# ╔═╡ 110345ef-c438-4abd-8b09-1fb057f43839
@bind sl_2 PlutoUI.Slider(1:1:size(T1map,3), default=ceil(Int,size(T1map,3)/2))

# ╔═╡ 91bb3f1e-a1d5-437f-8a02-57e81d4951de
	let
		f=Figure()
		ax=Axis(f[1,1],title = "MP2RAGE",aspect=SpatialResolution[2]/SpatialResolution[1])
		heatmap!(ax,abs.(MP2[:,:,sl_2]))
		hidedecorations!(ax)
		ax=Axis(f[1,2],title = "MP2RAGE",aspect=SpatialResolution[2]/SpatialResolution[1])
		h=heatmap!(ax,abs.(T1map[:,:,sl_2]),aspect = 1)
		hidedecorations!(ax)
		Colorbar(f[1,end+1],h,label = "T1 [ms]")
		f
	end

# ╔═╡ 7c17ca8c-76b6-4188-b654-f74ebf3d8897
md"# Export Data to nifti"

# ╔═╡ a12f0b7d-7c9f-4745-85ab-f58a7432a66c
begin
	ni = NIVolume(T1map,voxel_size =  SpatialResolution)
	niwrite(ni_folder*"/T1_"*ni_basename*".nii",ni)

	ni = NIVolume(MP2,voxel_size =  SpatialResolution)
	niwrite(ni_folder*"/MP2_"*ni_basename*".nii",ni)

	ni = NIVolume(abs.(Imean),voxel_size =  SpatialResolution)
	niwrite(ni_folder*"/TI_"*ni_basename*".nii",ni)
end
...
manifest_format = "2.0"
project_hash = "b3373c219f4bd9000bf5133ce4289e5a04ee2721"
...
[[deps.MRICoilSensitivities]]
deps = ["FFTW", "FLoops", "LinearAlgebra", "MRIBase", "Printf", "Reexport"]
git-tree-sha1 = "6c839d4d92e91eda49e64a54fb76f385f60c8ef3"
...
[[deps.MRISimulation]]
deps = ["LinearAlgebra", "MRIBase", "MRIOperators", "Printf", "ProgressMeter", "Reexport", "StatsBase"]
git-tree-sha1 = "6a013aaefd16b288bf40e11cd500d1aae9b4598f"
...
[[deps.NIfTI]]
deps = ["Base64", "GZip", "MappedArrays", "Mmap", "Test"]
...
[[deps.QuantitativeMRI]]
deps = ["LsqFit", "MRISimulation", "Optim", "Statistics"]
git-tree-sha1 = "5b4f53496be48a5b3e4d75ce185d0710147bef02"
uuid = "659767e3-31a7-4dc1-8563-6e03f484b231"
version = "0.1.2"
