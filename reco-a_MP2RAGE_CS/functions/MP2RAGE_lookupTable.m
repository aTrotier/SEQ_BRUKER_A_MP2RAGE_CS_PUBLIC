...
if ~isreal(IMs4D)
    MP2RAGE=real(conj(IMs4D(:,:,:,1,:)).*IMs4D(:,:,:,2,:)./(abs(IMs4D(:,:,:,1,:)).^2+abs(IMs4D(:,:,:,2,:)).^2));
else
    error('4D dataset are not complex');
end
...
struct_out.LUT.MP2RAGE = MP2RAGE;
struct_out.LUT.Signal = lookuptable;
struct_out.LUT.T1 = T1;
struct_out.LUT.struct_MP2RAGE = struct_MP2RAGE;
end
